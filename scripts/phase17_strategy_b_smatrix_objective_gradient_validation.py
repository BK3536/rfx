"""Phase XVII Strategy B native S-matrix objective gradient validation artifact."""
...
FD_REL_THRESHOLD = 0.15
FD_ABS_THRESHOLD = 5e-5
...
def _make_two_port_sim() -> Simulation:
    sim = Simulation(
        freq_max=5e9,
...
def _value_match_record(*, n_steps: int, freqs: jnp.ndarray) -> dict[str, Any]:
    ...
    return {
        "case": "one_port_pec_s11_mse",
        "n_steps": n_steps,
        "checkpoint_every": checkpoint_every,
        "freqs_hz": np.asarray(freqs),
        "expected_from_phase16_native_smatrix": float(expected),
        "phase17_objective": float(actual),
        "abs_error": abs_error,
        "rel_error": rel_error,
        "value_match_valid": abs_error <= VALUE_ABS_THRESHOLD or rel_error <= VALUE_REL_THRESHOLD,
    }


def _gradient_record(*, n_steps: int, freqs: jnp.ndarray) -> dict[str, Any]:
    ...
    finite_valid = bool(np.isfinite(grad_np).all())
    ...
    return {
        "case": "two_port_pec_weighted_s11_s21",
        "n_steps": n_steps,
        "checkpoint_every": checkpoint_every,
        "freqs_hz": np.asarray(freqs),
        "gradient_norm": norm,
        "gradient_finite_valid": finite_valid,
...
def _sidecar_stop_gradient_record(*, n_steps: int, freqs: jnp.ndarray) -> dict[str, Any]:
    ...
    return {
        "case": "phase16_default_sidecar_stop_gradient",
        "gradient": float(grad),
...
def _unsupported_scope_record(*, n_steps: int, freqs: jnp.ndarray) -> dict[str, Any]:
    ...
    inputs = sim.build_hybrid_phase1_inputs(n_steps=n_steps, s_param_freqs=freqs)
    request = native_minimize_s11_request(freqs)
    phase16 = sim.forward_hybrid_phase1_from_inputs(inputs, strategy="b", checkpoint_every=checkpoint_every)
    ...
    return {
        "case": "cpml_objective_gradient_gate",
        "phase16_cpml_value_supported": phase16.s_params is not None and bool(np.isfinite(np.asarray(phase16.s_params)).all()),
        "phase17_objective_supported": bool(report.supported),
        "reason_text": report.reason_text,
        "unsupported_scope_fail_closed_valid": (not report.supported) and "CPML remains value-only" in report.reason_text,
    }


def _native_provenance_record(*, n_steps: int, freqs: jnp.ndarray) -> dict[str, Any]:
    # Patch standard S-parameter entry points while running the objective so the
    # artifact carries the same anti-masquerade evidence as the unit tests.
    sim = _make_two_port_sim()
    checkpoint_every = max(1, n_steps // 3)
    inputs = sim.build_hybrid_phase1_inputs(n_steps=n_steps, s_param_freqs=freqs)
    ...
    def _fail_standard(*_args, **_kwargs):
        ...
        raise AssertionError("standard S-parameter extraction must not run in Phase XVII native objective")
    ...
    return {
        "case": "native_provenance",
        "objective_source": "rfx.hybrid_adjoint.run_phase17_strategy_b_native_smatrix_objective",
        "standard_extractor_reference_used_as_native_output": False,
...
def run_validation(*, n_steps: int = 48) -> dict[str, Any]:
    ...
    gates = {
        "objective_value_matches_native_smatrix_valid": value["value_match_valid"],
        "gradient_finite_valid": gradient["gradient_finite_valid"],
        "gradient_nonzero_valid": gradient["gradient_nonzero_valid"],
        "finite_difference_correlation_valid": gradient["finite_difference_correlation_valid"],
        ...
        "phase16_regression_valid": unsupported["phase16_cpml_value_supported"],
    }
    all_gates = all(gates.values())
    return {
        "schema_version": "phase17_strategy_b_smatrix_objective_gradient_v1",
        "overall_status": (
            "phase17_strategy_b_smatrix_objective_validated"
            if all_gates
            else "phase17_strategy_b_smatrix_objective_failed"
        ),
        "supported_scope": {
            "grid": "uniform",
            "boundary": "pec",
