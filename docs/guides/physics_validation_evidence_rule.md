...
| `add_waveguide_port` | Strongest current port candidate because empty-guide, PEC-short, passivity, Airy, dump, and external-solver artifacts exist. Documentation must cite exact gates and keep stale status text reconciled. |
| `add_coaxial_port` | Geometry/source helper plus M17/M22 analytic TEM oracle/sweep, M21 diagnostic coaxial-gap V/I replay, M35 narrow gap/openEMS comparison, M61 synthetic TEM reference-plane V/I extraction oracle, M62 synthetic Cartesian plane adapter oracle, M63 blocked real-FDTD DFT-plane replay diagnostic, M64 blocked axial reference-plane sweep, M65 structural signal-path audit, and M66 annular outer-shell stamp fix/blocked post-fix sweep, and M67 distributed TEM plane-source prototype only; still no promoted S-parameter claim until real-FDTD calibrated TEM reference-plane extraction, matched/open/short/load gates, and external crossval envelope exist. |
