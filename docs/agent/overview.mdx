...
Agents should treat the **uniform Cartesian Yee RF workflow** as the current
claims-bearing reference lane. Non-uniform graded-z, distributed, subgridding,
and Floquet/Bloch workflows should be treated as shadow or experimental lanes
unless the task explicitly requires them and the current support matrix allows
...
For thin-substrate graded-z workflows, prefer to treat them as a
**shadow-qualification path** rather than the default first choice.
