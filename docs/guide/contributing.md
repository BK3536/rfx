...
All tests must pass before submitting a change. The CI pipeline runs the
full suite on every push.

## Code Style

rfx does not enforce a linter yet. Follow the conventions visible in the
existing codebase:
...
3. **Verify.** Run the full test suite to ensure nothing is broken:
   ```bash
   pytest tests/ -x -q
   ```
...
the existing [documentation](index.md).
