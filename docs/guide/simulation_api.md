...
The `Simulation` builder is the main high-level interface in `rfx`.
...
sim = Simulation(
    freq_max=10e9,
    domain=(0.12, 0.04, 0.02),   # metres
    boundary="cpml",             # "cpml" or "pec"
    cpml_layers=12,
    dx=0.002,                    # optional; auto-derived if omitted
    mode="3d",                   # "3d", "2d_tmz", or "2d_tez"
    dz_profile=None,             # optional non-uniform z cell sizes
)
```

If `dz_profile` is provided, the z-domain can be defined from the profile.
...
sim.add_port(
    ...
    component="ez",
    impedance=50.0,
    extent=0.0016,   # wire port along z
