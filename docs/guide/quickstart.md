...
This guide uses the **current v1.0 API** and avoids older pre-v1 examples that
...
print(cfg.summary())   # includes dx, CPML, source type, and dz_profile if enabled
...
- [Simulation API](simulation_api.md) — current method signatures and result fields
- [Sources & Ports](sources_ports.md) — when to use `add_source()` vs `add_port()`
- [Non-Uniform Mesh](nonuniform_mesh.md) — `dz_profile` and thin-substrate workflow
- [Validation](validation.md) — what is strongly benchmarked today
