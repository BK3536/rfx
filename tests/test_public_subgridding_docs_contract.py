...
def test_readme_marks_subgridding_as_non_reference_lane():
    ...
    assert "experimental arbitrary-box + selected reflector/periodic/CPML subsets; proxy benchmark only" in text
...
def test_public_subgridding_guide_is_scoped_and_experimental():
    ...
    assert "experimental axis-aligned arbitrary-box lane" in text
    assert "selected" in text and "reflector/periodic" in text
    assert "bounded CPML" in text
    ...
    assert "calibrated S-parameters" in text
...
def test_public_indexes_do_not_overclaim_subgridding():
    for path in (PUBLIC_INDEX, GUIDE_INDEX):
        text = _text(path)
        assert "experimental arbitrary-box" in text
        assert "reflector/periodic/CPML" in text
...
def test_support_boundaries_publish_experimental_sbp_sat_subset():
    ...
    assert "all-PEC boundary, selected reflector/periodic subsets, and a bounded CPML absorbing subset" in text
    assert "one axis-aligned refinement box only; CPML cases require an absorber-pad guard" in text
    assert "UPML + SBP-SAT subgridding" in text
    assert "CPML refinement box inside the absorber guard" in text
