...
resonant frequency of a PEC rectangular cavity.
...
Physics:
  ...
  We measure the fundamental TE_101 resonance of the cavity via
...
def main():
    ...
    # --- Build PEC cavity simulation (no iris — clean TE101 validation) ---
    sim = Simulation(
        freq_max=f_max,
    ...
    # Use probe 1 (far side) for cleaner ringdown — less source contamination
    probe_idx = 1 if ts.shape[1] >= 2 else 0
    signal = ts[:, probe_idx]

    # Use harminv_from_probe which does proper bandpass filtering and
    # source-decay windowing to avoid picking up the excitation pulse.
    source = GaussianPulse(f0=f_te101, bandwidth=0.5)
    source_decay = source.t0 + 4 * source.tau

    # Narrow search range around the analytical TE101 frequency
    modes = harminv_from_probe(
        signal,
        dt,
        freq_range=(f_te101 * 0.7, f_te101 * 1.3),
        source_decay_time=source_decay,
        min_Q=10.0,
    )

    # Also compute start_idx for FFT plotting below
    ...
    # --- Find the mode closest to analytical TE101 ---
    f_harminv = None
    best_mode = None
    if modes:
        # Pick the mode closest to the analytical frequency (not just strongest)
        best_mode = min(modes, key=lambda m: abs(m.freq - f_te101))
